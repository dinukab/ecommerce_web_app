...
export const getAllCategories = async (req, res) => {
  try {
    const { storeId = 'STORE-2025-001' } = req.query;
    const categories = await Category.find({ storeId }).sort({ name: 1 }).lean();
...
export const getCategoryBySlug = async (req, res) => {
  try {
    ...
    let category = await Category.findOne({ slug: reqSlug, storeId }).lean();
    if (!category) {
